...
/*[AM /DC] --start--*/
/*
 * Neutrino self-interaction tight-coupling approximation (nu_tca):
 * switch off when tau_nu/tau_h > trigger_tau_nu_over_tau_h
 * and tau_nu/tau_k > trigger_tau_nu_over_tau_k.
 * Decrease to switch off earlier (weaker coupling regime).
 */
...
/*
 * Neutrino full Boltzmann hierarchy trigger:
 * switch on fluid approximation only when tau_nu/tau_k > this value.
 * Increase to keep the full hierarchy active longer.
 */
class_precision_parameter(full_hierarchy_trigger_tau_nu_over_tau_k,double,1.e4)
/**
 * Neutrino TCA initial time trigger:
 * start integrating only when aH/Gamma_nu < start_small_k_at_tau_nu_over_tau_h.
 */
class_precision_parameter(start_small_k_at_tau_nu_over_tau_h,double,2.e-5)
/*[AM /DC] --end--*/
...
/**
 * Neutrino self-interaction collision integral data files.
 * Newtonian gauge uses the 11-bin file; synchronous gauge uses the 5-bin file.
 */
