...
cdef class Class:
    ...
    @property
    def G_eff_nu(self):
      """Return the effective neutrino self-interaction coupling constant G_eff_nu
      in units of MeV^-2. Returns 0 for non-interacting (standard) neutrinos."""
      return CallableFloat(self.ba.G_eff_nu)
    @property
    def log10_G_eff_nu(self):
      """Return log10 of the effective neutrino self-interaction coupling constant
      G_eff_nu [MeV^-2]. Returns -12 (effectively zero) for standard neutrinos."""
      return CallableFloat(self.ba.log10_G_eff_nu)
    @property
    def is_interacting_nu(self):
      """Return True if neutrino self-interaction is active (G_eff_nu != 0)."""
      return bool(self.ba.interacting_nu != 0.)
    # [AM /DC] --end-- # 
    ...
    def set_default(self):
        _pars = {
            "output":"tCl mPk",
            "base_path": os.path.dirname(os.path.dirname(os.path.abspath(__file__))),}## [AM / DC] INCLUDE HERE the folder place or change it in your ini file.
        self.set(**_pars)
