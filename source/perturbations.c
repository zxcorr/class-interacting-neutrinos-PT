...
int perturbations_init(
  ...
  /* [AM / DC] --start-- */
  /*initialize collision array pointers to NULL so perturbations_free
    is safe even when interacting_nu = 0 and arrays were never allocated */
  ...
  /* [AM | DC]: load neutrino self-interaction collision tables */
  if (pba->interacting_nu != 0.) {
    ...
    class_call(perturb_collision_interpolate_ncdm(ppr,pba,ppt),
               ppt->error_message,
               ppt->error_message);
  }
...
int perturbations_free(
  ...
  /* [AM / DC] start: free neutrino self-interaction collision tables */
  //if (pba->interacting_nu != 0.) {
  ...
  /* [AM / DC]: free neutrino self-interaction collision tables if allocated */
  class_call(perturbations_collision_free(ppt),
             ppt->error_message,
             ppt->error_message);
  /* [AM / DC] end */    
...
int perturbations_workspace_init(
  ...
  if (_scalars_) {
    ...
    /* [AM / DC]: neutrino TCA approximation index */
    class_define_index(ppw->index_ap_nu_tca,(pba->interacting_nu!=0.),index_ap,1);      
  }
  ...
  if (_scalars_) {
    ...
    /* [AM / DC]: initialise neutrino TCA — start in tight-coupling */
    if (pba->interacting_nu != 0.) {
...
int perturbations_solve(
  ...
  /**/
  /* [AM / DC] --start-- also check that neutrinos are in TCA regime at initial time */
...
int perturbations_vector_init(
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      /* [AM / DC] --start-- case of switching off neutrino TCA approximation.
         Initialise ur shear and higher multipoles, ncdm shear and higher multipoles. */
      if (pba->interacting_nu != 0.) {
      if ((pa_old[ppw->index_ap_nu_tca] == (int)nu_tca_on) &&
          (ppw->approx[ppw->index_ap_nu_tca] == (int)nu_tca_off)) {

        if (ppt->perturbations_verbose > 2)
          fprintf(stdout,"Mode k=%e: switch off neutrino TCA at tau=%e\n",k,tau);

        /* --- photons (copy delta, theta; shear and higher only if tca_off) --- */
        if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
        ...
        /* --- massless neutrinos (ur): set shear and l>=3 from TCA solution --- */
        if (pba->has_ur == _TRUE_) {
          double tau_ur_ic = 1./ppw->pvecback[pba->index_bg_Gamma_ur];
          ...
          /* use tca_shear_ur stored in workspace by last derivs call */
          ppv->y[ppv->index_pt_shear_ur] = ppw->tca_shear_ur;  
          //ppv->y[ppv->index_pt_shear_ur] =
           // tau_ur_ic * 4./15. *
           // (ppv->y[ppv->index_pt_theta_ur]/k + metric_shear/k) /
           // ppt->alpha_ell[2];
          ppv->y[ppv->index_pt_l3_ur] =
            3./7. * k * tau_ur_ic * ppv->y[ppv->index_pt_shear_ur] / ppt->alpha_ell[3];
          ppv->y[ppv->index_pt_delta_ur+4] =
            4./9. * k * tau_ur_ic * ppv->y[ppv->index_pt_l3_ur] / ppt->alpha_ell[4];
          for (l = 5; l <= ppv->l_max_ur; l++)
            ppv->y[ppv->index_pt_delta_ur+l] = 0.;
        }

        /* --- massive neutrinos (ncdm): set shear and higher from TCA solution --- */
        if (pba->has_ncdm == _TRUE_) {
          double tau_ncdm_ic = 1./ppw->pvecback[pba->index_bg_Gamma_ncdm1];
          int l_max_nu_tca = 1;
          int index_pt_ic = 0;
          for (n_ncdm = 0; n_ncdm < ppv->N_ncdm; n_ncdm++) {
            for (index_q = 0; index_q < ppv->q_size_ncdm[n_ncdm]; index_q++) {
              double q_ic    = pba->q_ncdm[n_ncdm][index_q];
              double eps_ic  = sqrt(q_ic*q_ic +
                                    ppw->pvecback[pba->index_bg_a]*ppw->pvecback[pba->index_bg_a]*
                                    pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]);
              double qk_ic   = q_ic*k/eps_ic;
              double dlnf_ic = pba->dlnf0_dlnq_ncdm[n_ncdm][index_q];
              for (l = 0; l <= ppv->l_max_ncdm[n_ncdm]; l++) {
                if (l < 2) {
                  ppv->y[ppv->index_pt_psi0_ncdm1+index_pt_ic] =
                    ppw->pv->y[ppw->pv->index_pt_psi0_ncdm1 +
                               index_pt_ic - index_q*(ppv->l_max_ncdm[n_ncdm]-l_max_nu_tca)];
                }
                else if (l == 2) {
                  /* [AM / DC] --start-- metric_shear = k^2*alpha (sync) or 0 (newt), computed from pvecmetric */
                  double ms_ic = (ppt->gauge == synchronous) ?
                    k*k*ppw->pvecmetric[ppw->index_mt_alpha] : 0.;
                  ppv->y[ppv->index_pt_psi0_ncdm1+index_pt_ic] =
                    2./5.*k*tau_ncdm_ic *
                    (q_ic/eps_ic * ppv->y[ppv->index_pt_psi0_ncdm1+index_pt_ic-1]
                     - dlnf_ic*ms_ic/3./k) /
                    ppt->C_ell_ncdm[n_ncdm][2*ppv->q_size_ncdm[n_ncdm]+index_q];
                  /* [AM / DC] --end--  */                     
                }
                else if (l > 2 && l < 5) {
                  ppv->y[ppv->index_pt_psi0_ncdm1+index_pt_ic] =
                    (double)l/(l+1.) * qk_ic * tau_ncdm_ic *
                    ppv->y[ppv->index_pt_psi0_ncdm1+index_pt_ic-1] /
                    ppt->C_ell_ncdm[n_ncdm][l*ppv->q_size_ncdm[n_ncdm]+index_q];
                }
                else {
                  ppv->y[ppv->index_pt_psi0_ncdm1+index_pt_ic] = 0.;
                }
                index_pt_ic++;
              }
            }
...
int perturbations_approximations(
  ...
  /* [AM / DC] --start-- neutrino interaction timescale --end--*/
  ...
  if (_scalars_) {
    ...
    /* [AM / DC] --start-- fluid approximation logic depends on whether neutrinos interact */
    if (pba->interacting_nu == 0.) {

      /* [AM / DC] Non-interacting case: standard CLASS logic */
      if (pba->has_ur == _TRUE_) {
...
int perturbations_derivs(double tau,
  ...
  /* [AM / DC] --start-- neutrino interaction variables */
  double tau_nu = 0., tau_ur = 0., tau_ncdm = 0.;
  double tca_shear_ur = 0., tca_psi2_ncdm1 = 0.;
  int l_max_ur_int = 17;    
  /* [AM / DC] --end-- */    
  ...
  /* [AM/DC] [start] test to reset dy (it was not zero-initialized)*/
  int index_pt;
  for (index_pt = 0; index_pt < ppw->pv->pt_size; index_pt++) {
  dy[index_pt] = 0.0;
  }    
  /* [AM/DC] [end]*/  
  ...
  if (_scalars_) {
    ...
    /* [AM / DC] --start-- split into non-interacting and interacting branches */
    if (pba->has_ur == _TRUE_) {
      if (pba->interacting_nu == 0.) {
        /** Non-interacting branch: standard CLASS equations -- radiation streaming approximation is off*/
        if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
          /** - -----> ur density **/
          ...
          if (ppw->approx[ppw->index_ap_ufa] == (int)ufa_off) {
            ...
            /* - -----> exact ur l=3 */
            l = 3;
            dy[pv->index_pt_l3_ur] = k/(2.*l+1.)*(l*2.*s_l[l]*s_l[2]*y[pv->index_pt_shear_ur]-(l+1.)*s_l[l+1]*y[pv->index_pt_l3_ur+1]);
            
            /* - -----> exact ur l>3 */ 
            for (l = 4; l < pv->l_max_ur; l++) {
              dy[pv->index_pt_delta_ur+l] = k/(2.*l+1)*(l*s_l[l]*y[pv->index_pt_delta_ur+l-1]-(l+1.)*s_l[l+1]*y[pv->index_pt_delta_ur+l+1]);
            }
              
            /** - -----> exact ur lmax_ur */  
            l = pv->l_max_ur;
            dy[pv->index_pt_delta_ur+l] = k*(s_l[l]*y[pv->index_pt_delta_ur+l-1]-(1.+l)*cotKgen*y[pv->index_pt_delta_ur+l]);
          }
          else {
            /* [DC] TBC: curvature? */  
            /* - -----> in fluid approximation (ufa): only ur shear needed */  
            if (ppr->ur_fluid_approximation == ufa_mb)/* a la Ma & Bertschinger */
              dy[pv->index_pt_shear_ur] = -3./tau*y[pv->index_pt_shear_ur]+2./3.*(y[pv->index_pt_theta_ur]+metric_shear);
            if (ppr->ur_fluid_approximation == ufa_hu)/* a la Hu */
              dy[pv->index_pt_shear_ur] = -3.*a_prime_over_a*y[pv->index_pt_shear_ur]+2./3.*(y[pv->index_pt_theta_ur]+metric_shear);
            if (ppr->ur_fluid_approximation == ufa_CLASS) /* a la CLASS */
              dy[pv->index_pt_shear_ur] = -3./tau*y[pv->index_pt_shear_ur]+2./3.*(y[pv->index_pt_theta_ur]+metric_ufa_class);
          }
        }
      }/** [AM / DC] end of non-interacting block*/
      else {/* [AM / DC] Interacting branch */     
        ...
        if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
          ...
          if (ppw->approx[ppw->index_ap_nu_tca] == (int)nu_tca_off) {/* [AM / DC] ----> if ur TCA approximation is off */
            /* [l=1] -----> ur velocity */
            dy[pv->index_pt_theta_ur] =
              // standard term with extra coefficient (3 ceff2_ur), normally equal to one  
              k2*(ppt->three_ceff2_ur*y[pv->index_pt_delta_ur]/4.-s2_squared*y[pv->index_pt_shear_ur]) + metric_euler
            ...
            if (ppw->approx[ppw->index_ap_ufa] == (int)ufa_off) {
              ...
              dy[pv->index_pt_shear_ur] =
                0.5*(// standard term
                     ...
                     - 2.*ppt->alpha_ell[2]*y[pv->index_pt_shear_ur]/tau_ur);/* [AM / DC] */
              /* [l=3] -----> exact ur l=3 */
              l = 3;
              dy[pv->index_pt_l3_ur] = k/(2.*l+1.)*(l*2.*s_l[l]*s_l[2]*y[pv->index_pt_shear_ur]-(l+1.)*s_l[l+1]*y[pv->index_pt_l3_ur+1])
                // neutrino interaction term  
                - ppt->alpha_ell[l]*y[pv->index_pt_l3_ur]/tau_ur;/* [AM / DC] */
              /* [l>3] -----> exact ur l>3 */ 
              if (pv->l_max_ur < l_max_ur_int) {/* [AM / DC] */
                for (l = 4; l < pv->l_max_ur; l++) {
                  // printf("alpha_%i %.8f\n",l,ppt->alpha_ell[l]);    
                  dy[pv->index_pt_delta_ur+l] = k/(2.*l+1)*(l*s_l[l]*y[pv->index_pt_delta_ur+l-1]-(l+1.)*s_l[l+1]*y[pv->index_pt_delta_ur+l+1])
                      // neutrino interaction term
                    - ppt->alpha_ell[l]*y[pv->index_pt_delta_ur+l]/tau_ur;/* [AM / DC] */
                }
                /* [l=lmax_ur] -----> exact ur lmax_ur */  
                l = pv->l_max_ur;
                dy[pv->index_pt_delta_ur+l] = k*(s_l[l]*y[pv->index_pt_delta_ur+l-1]-(1.+l)*cotKgen*y[pv->index_pt_delta_ur+l])
                  // neutrino interaction term  
                  - ppt->alpha_ell[l]*y[pv->index_pt_delta_ur+l]/tau_ur;/* [AM / DC] */
                ...
                /* [l>3] Collision term becomes constant at l_max_ur_int -- alpha_ell is capped at l_max_ur_int */
                ...
                for (l = l_max_ur_int; l < pv->l_max_ur; l++) {
                  ...
                  dy[pv->index_pt_delta_ur+l] = k/(2.*l+1)*(l*s_l[l]*y[pv->index_pt_delta_ur+l-1]-(l+1.)*s_l[l+1]*y[pv->index_pt_delta_ur+l+1])
                   // neutrino interaction term  
                    - ppt->alpha_ell[l_max_ur_int]*y[pv->index_pt_delta_ur+l]/tau_ur;/* [AM / DC] */
                }
                  /* [l=lmax_ur] -----> exact ur lmax_ur */  
                l = pv->l_max_ur;
                dy[pv->index_pt_delta_ur+l] = k*(s_l[l]*y[pv->index_pt_delta_ur+l-1]-(1.+l)*cotKgen*y[pv->index_pt_delta_ur+l])
              ...
              /*  [AM / DC] UFA with collision correction -- in fluid approximation (ufa): only ur shear needed */
              
              /* a la Ma & Bertschinger */
              // Forcing all cases  
              if (ppr->ur_fluid_approximation == ufa_mb)
                dy[pv->index_pt_shear_ur] = -3.*(1./tau+ppt->ufa_corrections*ppt->alpha_ell[2]/tau_ur)*y[pv->index_pt_shear_ur]
                  +2./3.*(y[pv->index_pt_theta_ur]+metric_shear);
              /* a la Hu */  
              if (ppr->ur_fluid_approximation == ufa_hu)
                dy[pv->index_pt_shear_ur] = -3.*(a_prime_over_a+ppt->ufa_corrections*ppt->alpha_ell[2]/tau_ur)*y[pv->index_pt_shear_ur]
                  +2./3.*(y[pv->index_pt_theta_ur]+metric_shear);
              /* a la CLASS */  
    ...
    /* [AM / DC] --start-- split into non-interacting and interacting branches */
    // [DC] TBC: curvature in all ncdm  
    if (pba->has_ncdm == _TRUE_) {
      idx = pv->index_pt_psi0_ncdm1;
      /* Non-interacting branch: standard CLASS equations*/        
      if (pba->interacting_nu == 0.) {
        /* [CASE 1] use a fluid approximation (ncdmfa)  */          
        // [DC] TBC: curvature          
        if (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on) {
          /** - -----> loop over species */
          for (n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
            ...
            if (ppr->ncdm_fluid_approximation == ncdmfa_mb) { ceff2_ncdm=ca2_ncdm; cvis2_ncdm=3.*w_ncdm*ca2_ncdm; }
            if (ppr->ncdm_fluid_approximation == ncdmfa_hu) { ceff2_ncdm=ca2_ncdm; cvis2_ncdm=w_ncdm; }
            if (ppr->ncdm_fluid_approximation == ncdmfa_CLASS) { ceff2_ncdm=ca2_ncdm; cvis2_ncdm=3.*w_ncdm*ca2_ncdm; }
            /* - -----> exact continuity equation */ 
            dy[idx] = -(1.0+w_ncdm)*(y[idx+1]+metric_continuity)-3.0*a_prime_over_a*(ceff2_ncdm-w_ncdm)*y[idx];
            /* - -----> exact euler equation */  
            dy[idx+1] = -a_prime_over_a*(1.0-3.0*ca2_ncdm)*y[idx+1]+ceff2_ncdm/(1.0+w_ncdm)*k2*y[idx]-k2*y[idx+2]+metric_euler;
          ...
          /*  [CASE 2] use exact equation (Boltzmann hierarchy on momentum grid) */
          /* - -----> loop over species */  
          for (n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
            /* - -----> loop over momentum */  
        ...
        /*  [AM / DC] --start-- Interacting branch */
        /* [CASE 1] use a fluid approximation (ncdmfa)  */
        //TBC: curvature  
        if (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on) {
          /* - -----> loop over species */ 
          for (n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
            ...
            /* - -----> exact euler equation */  
            dy[idx+1] = -a_prime_over_a*(1.0-3.0*ca2_ncdm)*y[idx+1]+ceff2_ncdm/(1.0+w_ncdm)*k2*y[idx]-k2*y[idx+2]+metric_euler;
            /* Forcing all approximations */
            /* collision correction to shear damping --- different ansatz for approximate shear derivative */
            if (ppr->ncdm_fluid_approximation == ncdmfa_mb)
              dy[idx+2] = -3.0*(a_prime_over_a*(2./3.-ca2_ncdm-pseudo_p_ncdm/p_ncdm_bg/3.)+1./tau+ppt->ncdmfa_corrections*ppt->alpha_ell[2]/tau_ncdm)*y[idx+2]
            ...
            if (ppr->ncdm_fluid_approximation == ncdmfa_CLASS)
              /* printf("1./tau: %e\n",1./tau); */    
              /* printf("ppt->alpha_ell[2]/tau_ncdm: %e\n",ppt->alpha_ell[2]/tau_ncdm); */  
              dy[idx+2] = -3.0*(a_prime_over_a*(2./3.-ca2_ncdm-pseudo_p_ncdm/p_ncdm_bg/3.)+1./tau+ppt->ncdmfa_corrections*ppt->alpha_ell[2]/tau_ncdm)*y[idx+2]
        ...
        else {/* [CASE 2] use exact equation (Boltzmann hierarchy on momentum grid) */
          /* ----> TCA approximation for neutrinos is on */  
          if (ppw->approx[ppw->index_ap_nu_tca] == (int)nu_tca_on) {
            /* -----> loop over species */  
            /* DC: nu_tca_on: only delta and theta; psi2 from TCA expression */
            for (n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
              for (index_q=0; index_q < pv->q_size_ncdm[n_ncdm]; index_q++) {
                dlnf0_dlnq  = pba->dlnf0_dlnq_ncdm[n_ncdm][index_q];
                q           = pba->q_ncdm[n_ncdm][index_q];
                ...
                /* [l=0] -----> ncdm density for given momentum bin */  
                dy[idx] = -qk_div_epsilon*y[idx+1]+metric_continuity*dlnf0_dlnq/3.;
                /* [l=2] -----> ncdm TCA "shear" for given momentum bin */  
                tca_psi2_ncdm1 = k*tau_ncdm*2./5.*(q*y[idx+1]/epsilon - metric_shear*dlnf0_dlnq/3./k)
                                 /ppt->C_ell_ncdm[n_ncdm][2*pv->q_size_ncdm[n_ncdm]+index_q];
                /* [l=1] -----> ncdm velocity for given momentum bin */
                // printf("ppw->tca_psi2_ncdm1[n_ncdm][index_q]: %e\n",ppw->tca_psi2_ncdm1[n_ncdm][index_q]);                  
                dy[idx+1] = qk_div_epsilon/3.0*(y[idx]-2*s_l[2]*tca_psi2_ncdm1)-epsilon*metric_euler/(3*q*k)*dlnf0_dlnq;
                /** - -----> jump to next momentum bin or species */  
                idx += (pv->l_max_ncdm[n_ncdm]+1);
              }
            }
          }
          /* ----> TCA approximation for neutrinos is off, full hierarchy */    
          else {

            /* [AM / DC] nu_tca_off: full hierarchy with collision terms */
            for (n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
              /* - -----> loop over momentum */  
              for (index_q=0; index_q < pv->q_size_ncdm[n_ncdm]; index_q++) {
                /* - -----> define intermediate quantities */  
                ...
                /* [l=2]-----> shear for given momentum bin (with collision term) */
                ...
                /* [l>2]  ncdm l>2 for given momentum bin */
                for (l=3; l<pv->l_max_ncdm[n_ncdm]; l++) {
                  dy[idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)])
                    // interaction term  
                    -ppt->C_ell_ncdm[n_ncdm][l*pv->q_size_ncdm[n_ncdm]+index_q]*y[idx+l]/tau_ncdm;
                }
                /* [l=l_max] -----> ncdm lmax for given momentum bin (truncation as in Ma and Bertschinger)
                    but with curvature taken into account a la arXiv:1305.3261 */                  
                dy[idx+l] = qk_div_epsilon*y[idx+l-1]-(1.+l)*k*cotKgen*y[idx+l]
                  // interaction term  
                  -ppt->C_ell_ncdm[n_ncdm][l*pv->q_size_ncdm[n_ncdm]+index_q]*y[idx+l]/tau_ncdm;
...
int perturb_collision_C_ell(struct precision * ppr,
  ...
  int max_l_required = 0;

  /**Summary: */
  /* the following file is assumed to contain (apart from comments and blank lines):
     - the two numbers (num_q, num_ell) = number of values of BBN free parameters
     - three columns (q_collision, ell, C_ell) 
     - q_collision and ell are assumed to be arranged as:
     q_collision1 ell1 C_ell
     q_collision2 ell1 C_ell
     .....
     q_collision1 delatN2 C_ell
     q_collision2 ell2 C_ell
     .....
  */
  if (ppt->gauge == newtonian){
    printf("*****************************************************************************\n");
    printf("Results provided by integration in the Newtonian gauge are highly inaccurate!\n");
    printf("*****************************************************************************\n");
  ...
  if (ppt->gauge == synchronous){
    class_open(fA,ppr->interacting_C_ell_file_syn, "r",ppt->error_message);
  ...
  while (fgets(line,_LINE_LENGTH_MAX_-1,fA) != NULL) {
    ...
    if (left[0] > 39) {
      /* if the line contains data, we must interpret it. If
         (num_q, num_ell)=(0,0), the current line must contain
         their values. Otherwise, it must contain (q_collision, delatN,
         C_ell). */
      if ((num_q==0) && (num_ell==0)) {
        ...
        class_test((num_q <= 0) || (num_ell <= 0),
                   ppt->error_message,
                   "invalid collision table dimensions in file (num_q=%d, num_ell=%d)",num_q,num_ell);
        class_alloc(ppt->q_collision,num_q*sizeof(double),ppt->error_message);
        class_alloc(ppt->ell,num_ell*sizeof(int),ppt->error_message);
        ...
        // class_alloc(ppt->C_ell_at_q_collision,num_q*sizeof(double),ppt->error_message);
        // class_alloc(ppt->ddC_ell_at_q_collision,num_q*sizeof(double),ppt->error_message);
        expected_lines = num_q*num_ell;
        array_line=0;
      }
      else {
        /* read (q_collision, ell, C_ell) */
        class_test(array_line >= expected_lines,
                   ppt->error_message,
                   "too many data lines in collision table file");
        if (ppt->gauge == newtonian){
          class_test(sscanf(line,"%lg %d %lg",
  ...
  class_test(array_line != expected_lines,
             ppt->error_message,
             "collision table has inconsistent number of rows: expected %d, read %d",
             expected_lines,array_line);
  for (array_line=1; array_line<num_q; array_line++) {
    class_test(ppt->q_collision[array_line] <= ppt->q_collision[array_line-1],
               ppt->error_message,
               "q grid in collision table must be strictly increasing");
  }
  max_l_required = MAX(4,ppr->l_max_ncdm);
  class_test((num_ell-1) < max_l_required,
             ppt->error_message,
             "collision table does not reach enough multipoles: l_max=%d, required >= %d",
             num_ell-1,max_l_required);
  ppt->num_q_collision = num_q;
  ppt->num_ell_collision = num_ell;
  return _SUCCESS_;
}/* [AM / DC] --end-- */
...
int perturb_collision_alpha_ell(struct precision * ppr,
  ...
  while (fgets(line,_LINE_LENGTH_MAX_-1,fA) != NULL) {
    ...
    if (left[0] > 39) {
      ...
      if ((num_ell==0)) {
        ...
        class_test(num_ell <= 0,
                   ppt->error_message,
                   "invalid alpha table length (%d) in file %s",num_ell,ppr->interacting_alpha_ell_file);
        ...
        class_test(array_line >= expected_lines,
                   ppt->error_message,
                   "too many data lines in alpha_ell table file %s",ppr->interacting_alpha_ell_file);
  ...
  class_test(array_line != expected_lines,
             ppt->error_message,
             "alpha_ell table has inconsistent number of rows: expected %d, read %d",
             expected_lines,array_line);
  min_alpha_lmax = 4;
  class_test((num_ell-1) < min_alpha_lmax,
             ppt->error_message,
             "alpha_ell table does not reach enough multipoles: l_max=%d, required >= %d",
             num_ell-1,min_alpha_lmax);
  ppt->num_ell_alpha = num_ell;
  return _SUCCESS_;
}/* [AM / DC] --end-- */

/* [AM / DC] --start-- */
int perturb_collision_interpolate_ncdm(struct precision * ppr,
                                       struct background * pba,
                                       struct perturbations * ppt) {

  int n_ncdm,index_q,l,iq_low;
  double q,target_q,w;
  int nq;

  class_test((pba->has_ncdm == _FALSE_) || (pba->N_ncdm <= 0),
             ppt->error_message,
             "interacting neutrino mode requires at least one ncdm species");

  class_test((ppt->q_collision == NULL) || (ppt->C_ell == NULL),
             ppt->error_message,
             "collision table must be loaded before interpolation");

  class_alloc(ppt->C_ell_ncdm,pba->N_ncdm*sizeof(double*),ppt->error_message);
  ppt->C_ell_ncdm_size = pba->N_ncdm;
  for (n_ncdm=0; n_ncdm<pba->N_ncdm; n_ncdm++) {
    ppt->C_ell_ncdm[n_ncdm] = NULL;
  }
  ...
  for (n_ncdm=0; n_ncdm<pba->N_ncdm; n_ncdm++) {
    ...
    class_test(nq <= 0,
               ppt->error_message,
               "invalid ncdm q-grid size for species %d",n_ncdm);
    class_alloc(ppt->C_ell_ncdm[n_ncdm],ppt->num_ell_collision*nq*sizeof(double),ppt->error_message);

    for (l=0; l<ppt->num_ell_collision; l++) {
      for (index_q=0; index_q<nq; index_q++) {
        target_q = pba->q_ncdm[n_ncdm][index_q];
        if (target_q <= ppt->q_collision[0]) {
          ppt->C_ell_ncdm[n_ncdm][l*nq+index_q] = ppt->C_ell[l*ppt->num_q_collision];
          continue;
        }
        if (target_q >= ppt->q_collision[ppt->num_q_collision-1]) {
          ppt->C_ell_ncdm[n_ncdm][l*nq+index_q] = ppt->C_ell[l*ppt->num_q_collision + (ppt->num_q_collision-1)];
          continue;
        }

        iq_low = 0;
        while ((iq_low+1 < ppt->num_q_collision) && (ppt->q_collision[iq_low+1] < target_q)) {
          iq_low++;
        }
        q = ppt->q_collision[iq_low+1]-ppt->q_collision[iq_low];
        class_test(q <= 0.,
                   ppt->error_message,
                   "invalid collision q-grid spacing at index %d",iq_low);
        w = (target_q-ppt->q_collision[iq_low])/q;
        ppt->C_ell_ncdm[n_ncdm][l*nq+index_q] =
          (1.-w)*ppt->C_ell[l*ppt->num_q_collision+iq_low]
          +w*ppt->C_ell[l*ppt->num_q_collision+iq_low+1];
      }
    }
  }
...
int perturbations_collision_free(struct perturbations * ppt) {
  int n_ncdm;
  /* Deallocate neutrino self-interaction collision tables if allocated */
  if (ppt->q_collision != NULL) { free(ppt->q_collision);  ppt->q_collision = NULL; }
  if (ppt->ell != NULL)         { free(ppt->ell);          ppt->ell         = NULL; }
  if (ppt->C_ell != NULL)       { free(ppt->C_ell);        ppt->C_ell       = NULL; }
  if (ppt->ddC_ell != NULL)     { free(ppt->ddC_ell);      ppt->ddC_ell     = NULL; }
  if (ppt->ell_2 != NULL)       { free(ppt->ell_2);        ppt->ell_2       = NULL; }
  if (ppt->alpha_ell != NULL)   { free(ppt->alpha_ell);    ppt->alpha_ell   = NULL; }
  if (ppt->C_ell_ncdm != NULL) {
    for (n_ncdm=0; n_ncdm<ppt->C_ell_ncdm_size; n_ncdm++) {
