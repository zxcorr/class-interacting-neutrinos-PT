...
int background_functions(
  ...
  if (pba->has_ncdm == _TRUE_) {
    ...
    for (n_ncdm=0; n_ncdm<pba->N_ncdm; n_ncdm++) {
      ...
      /* [AM / DC] --start-- interaction rate for massive neutrinos */
      if (pba->interacting_nu != 0.) {
        ...
        pvecback[pba->index_bg_Gamma_ncdm1+n_ncdm] =
          a*pow(pba->G_eff_nu*1e-12,2)*pow(pba->T_cmb*pba->T_ncdm[n_ncdm]*_k_B_/_eV_/a,5)
          *_Mpc_over_m_*_eV_/(_h_P_*_c_);
        // printf("%e\n",pvecback[pba->index_bg_Gamma_ncdm1+n_ncdm]*pow(a,4));
     /* [AM / DC] --end-- */
      }        
    }
  }
  ...
  if (pba->has_ur == _TRUE_) {
    ...
    /* [AM / DC] --start-- interaction rate for massless neutrinos */
    if (pba->interacting_nu != 0.) {
      pvecback[pba->index_bg_Gamma_ur] =
        a*pow(pba->G_eff_nu*1e-12,2)*pow(pba->T_cmb*pow(4./11.,1./3.)*_k_B_/_eV_/a,5)
        //For standard neutrinos: T_ncdm[0] = (4/11)^(1/3) ≈ 0.7164.
        *_Mpc_over_m_*_eV_/(_h_P_*_c_);
    }      
    /* [AM / DC] --end-- */      
  }
...
int background_indices(
  ...
  /* [AM / DC] interaction rate index for ncdm */
  if (pba->interacting_nu != 0.)
  ...
  /* [AM / DC] interaction rate index for ur */
  if (pba->interacting_nu != 0.)
...
int background_output_titles(
  ...
  /* [AM / DC] interaction rate output columns */
  if (pba->interacting_nu != 0.) {
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    /* [AM / DC] --start-- store interaction rates */
    if (pba->interacting_nu != 0.) {
